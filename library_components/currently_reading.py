...
def manage_reading_dialog(supabase, book_entry):
    st.write(f"### {book_entry['title']}")
    
    tab1, tab2, tab3 = st.tabs(["✅ Mark as Read", "❌ DNF", "🗑️ Remove"])
...
def currently(supabase, df):
    st.subheader("Currently Reading")

    df = df.loc[:, ~df.columns.duplicated()].copy()

    df = df[df['status'].str.strip().str.title() == 'Currently Reading'].copy().reset_index(drop=True)
    if df.empty:
        st.info("Your Currently Reading list is empty.")
        return
    ...
    if event.selection.rows:
        ...
        manage_reading_dialog(supabase, selected_row.to_dict())
